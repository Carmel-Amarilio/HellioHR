...
const users = [
  ...
  {
    email: 'agent@hellio.hr',
    password: process.env.AGENT_PASSWORD || 'CHANGE_ME_BEFORE_PRODUCTION',
...
const candidates = [
  ...
  },
  {
    ...
    status: 'ACTIVE' as Status,
    extractedSummary: 'Senior Python developer with 7 years of experience in web application development using Django and PostgreSQL. Proven track record of delivering scalable backend solutions.',
    extractedExperience: 'Senior Backend Engineer at TechCorp (2020-Present): Led development of e-commerce platform serving 100K+ users. Built RESTful APIs and implemented database optimization strategies.\n\nBackend Developer at StartupXYZ (2017-2020): Developed microservices architecture using Django and Docker. Improved API response times by 40%.',
    extractedEducation: 'BS in Computer Science, State University (2015)\nCertified AWS Solutions Architect (2019)',
    extractionMethod: 'AWS Bedrock - Claude 3.5 Sonnet',
    extractionStatus: 'SUCCESS',
    lastExtractionDate: new Date().toISOString(),
    extractionPromptVersion: 'v1.0',
  },
  {
    id: 'cand-003',
    name: 'Carol Davis',
  ...
  for (const candidate of candidates) {
    ...
    await prisma.candidate.upsert({
      ...
      update: {
        ...
        ...(candidate.extractedSummary && { extractedSummary: candidate.extractedSummary }),
        ...(candidate.extractedExperience && { extractedExperience: candidate.extractedExperience }),
        ...(candidate.extractedEducation && { extractedEducation: candidate.extractedEducation }),
        ...(candidate.extractionMethod && { extractionMethod: candidate.extractionMethod }),
        ...(candidate.extractionStatus && { extractionStatus: candidate.extractionStatus }),
        ...(candidate.lastExtractionDate && { lastExtractionDate: candidate.lastExtractionDate }),
        ...(candidate.extractionPromptVersion && { extractionPromptVersion: candidate.extractionPromptVersion }),
      },
      create: {
        id: candidate.id,
        name: candidate.name,
